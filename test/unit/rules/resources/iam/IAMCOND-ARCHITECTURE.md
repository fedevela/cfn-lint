...
| IAMCOND-001, IAMCOND-003; all five issue 122 procedures; all issue 122 verifications | `test/unit/rules/resources/iam/` owns acceptance fixtures, inert Runner contracts, and requirement/logic/architecture traceability. | The exact reproduction and six-resource matrix enter through the public Runner seam. No fixture owns production behavior, and no production package depends on test artifacts. | The selectors pin selection continuity, registration, severity, and full finding prefixes. Malkhut owns skip removal and executable validation after the schema implementation is present. | Keep fixtures and selectors inert during this phase; activate them only in executable validation. |
...
- **Test seam:** the inert Runner-level issue 122 fixtures and selectors await
  Malkhut activation.
...
- **Contract:** the active shared-schema matrix and inert issue 122 Runner matrix
