...
## Issue 124 embodied implementation record
...
| IAMCOND-005; all six mapped procedures; both issue 124 verifications | `test/unit/rules/resources/iam/` and the paired good/bad IAM fixture directories own acceptance boundaries and requirement-to-verification-to-logic-to-architecture traceability. | Both object-form fixtures enter through the public Runner seam. The contract binds E3513 registration to the concrete prefix and shared Condition tail; production packages have no dependency on test artifacts. | The active malformed selector pins E3513 identity, error severity, and exact offending-member path. The active recognized selector pins the absence of all E3513 findings, thereby preserving both Condition acceptance and ECR statement semantics. | Keep the paired fixtures and focused Runner selectors as the executable IAMCOND-005 acceptance boundary. |
