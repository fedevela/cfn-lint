...
`test_iamcond_issue_123_contract.py`. The issue 124 selectors are active in
`test_iamcond_issue_124_contract.py`. Their node IDs preserve requirement, ECR
entry point, condition shape, and expected outcome.

The deterministic procedures for the established shared-schema and issue 122
through issue 124 obligations are recorded in `IAMCOND-PSEUDOCODE.md`. The
...
| `test_IAMCOND_005_missing_operator_` | `VALIDATE_ISSUE_124_ECR_REPOSITORY_POLICY`; `CONFIGURE_IAM_POLICY_RULE_FAMILY`; `VALIDATE_IAM_POLICY_DOCUMENT`; `VALIDATE_SHARED_CONDITION`; `MATCH_RECOGNIZED_CONDITION_OPERATOR` |
| `test_IAMCOND_005_recognized_nested_condition_` | `VALIDATE_ISSUE_124_ECR_REPOSITORY_POLICY`; `CONFIGURE_IAM_POLICY_RULE_FAMILY`; `VALIDATE_IAM_POLICY_DOCUMENT`; `VALIDATE_SHARED_CONDITION`; `MATCH_RECOGNIZED_CONDITION_OPERATOR`; `VALIDATE_CONDITION_OPERATOR_BODY` |
...
| IAMCOND-005 | `ResourceEcrPolicy.keywords`; E1101 provider-schema dispatch; `Policy.validate`; `policy_resource_ecr.json`; `policy.json#/definitions/Condition`; the paired issue 124 fixtures and active Runner selectors | The ECR repository-policy path converges on the shared closed Condition schema. The malformed fixture observes error-level E3513 at the exact offending member; the recognized fixture observes no E3513 finding while preserving ECR's optional Resource semantics. |
